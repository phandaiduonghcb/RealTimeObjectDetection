...
def plot_current_money(count_list,im,lines):
    class_name = lines[0]
    money = lines[1]
    
    n =0
    tongtien = 0
    for cls in count_list:
        name = cls[0]
        sl = cls[1]
        try:
          tongtien+= sl * int(money[class_name.index(name)])
        except:
          pass
    s = 'Tien: ' + str(tongtien)
    font = cv2.FONT_HERSHEY_SIMPLEX
    fontscale = int(max(im.shape[0],im.shape[1])/1024)+1
    org=(10*fontscale,50*fontscale)
    thickness = 2*fontscale
    color = (0, 255, 0)
    cv2.putText(im,s,org,font,fontscale,color,thickness)
