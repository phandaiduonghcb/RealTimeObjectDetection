...
def detect_images(s,d,net,ctx,lines):
  net.collect_params().reset_ctx(ctx[0])
  images = glob.glob(s + '/*.jpg')
  for hinh in images:
    t0 = time.time()
    x, image = gcv.data.transforms.presets.ssd.load_test(hinh,512)
    new_x = x.copyto(ctx[0])
    cid, score, bbox = net(new_x)
    print(f'Done. ({time.time() - t0:.3f}s)')
    ax = viz.cv_plot_bbox(image, bbox[0], score[0], cid[0],thresh=0.5, class_names=classes)
    plot_current_money(ax,cid[0],score[0],lines)
    name = hinh.split('/')[-1]
    if name == '':
      name = hinh.split('/')[-2]
    plt.imsave(os.path.join(d,name),ax)

s = '/content/test/images' # Folder chứa hình cần detect
d = '/content/result/' # Folder chứa hình đã detect
...
net.collect_params().reset_ctx(ctx[0])
detect_images(s,d,net,ctx,lines)
